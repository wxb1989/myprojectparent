...
public class NioHttpServer {
    ...
    private void handleWrite(SelectionKey key) {
        ...
        while (queue.size() > 0) {
            ...
            buffer.put(("HTTP/1.0 200 OK" + CR + LF).getBytes());
            buffer.put(("Content-Type: text/html; charset=UTF-8" + CR + LF).getBytes());
            buffer.put(("Content-Length: " + response.length() + CR + LF).getBytes());
//			buffer.putChar('\r');
//			buffer.putChar('\n');
            buffer.put((CR + LF).getBytes());
